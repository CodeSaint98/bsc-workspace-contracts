...
interface BUSDBasic{
   ...
   contract BUSDConvert{
    ...
    // decimals
      ...
      function convertBUSDToBNB(uint BUSDAmount) public payable {
            // Buy BUSD with BNB stored in the contract
            require(BUSD_Token.balanceOf(msg.sender)>BUSDAmount, 'You do not have this much BUSD');
           uint deadline = block.timestamp + 120;
           uint amountIn = BUSDAmount * 10 ** 18;
           uint amountOutMin = getEstimatedBNBPrice(BUSDAmount);
           require(BUSD_Token.transferFrom(msg.sender, address(this), amountIn), 'transferFrom failed');
           require(BUSD_Token.approve(address(PANCAKE_ROUTER), amountIn), 'approve failed');
           PANCAKE_ROUTER.swapExactTokensForETH(amountIn, amountOutMin, getPathForBUSDtoBNB(), msg.sender, deadline );
           BNBbalance += amountOutMin;
       }
